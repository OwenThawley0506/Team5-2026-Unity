...
public class UVGen : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (player.hasCanister == true && player.hasHammer == false && isPlayerInRange && Input.GetKeyDown(KeyCode.F) && isUVGenOn == false)
        {
        ...
        if (player.hasCanister == false && isPlayerInRange && Input.GetKeyDown(KeyCode.F) && isUVGenOn == false)
        {
            Debug.Log("Player does not have the right item to repair the UV generator.");
        }
        if (isUVGenOn == true && isPlayerInRange && Input.GetKeyDown(KeyCode.F))
        {
